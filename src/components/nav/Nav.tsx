...
const Nav = () => {
    return (
        <div className=' rounded-full bg-primaryy flex items-center justify-between  fixed bottom-4 m-auto left-0 right-0 w-fit z-50 px-4 py-2 shadow-lg '>  
           <div
            ...
            className={({ isActive }) =>
                `hover:scale-x-105 transition-all duration-300
                ${isActive ? "scale-125 rounded-full p-[4px] bg-blackk" : ""}`
            }
            >
            <button
                className="before:hidden hover:before:flex before:justify-center before:items-center before:h-4 before:text-[.6rem] before:px-1 before:content-['Todo'] before:bg-blackk before:text-secondaryy before:absolute before:-top-7 before:rounded-lg hover:-translate-y-5 cursor-pointer hover:scale-125 bg-secondaryy rounded-full p-2 px-3"
            >
                📝
            </button>
            </NavLink>
